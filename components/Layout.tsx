...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const isForm = location.pathname.includes('/new') || location.pathname.includes('/edit') || location.pathname === '/log-injection' || location.pathname === '/check-in';
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F8F9FB] dark:bg-stone-950 text-stone-900 dark:text-stone-100 overflow-hidden font-sans transition-colors duration-300">
      <main className="flex-1 overflow-y-auto no-scrollbar pb-28">
        <div className="max-w-md mx-auto min-h-full p-6 relative">
            {children}
        </div>
      </main>
