...
export const getNextScheduledDose = (compounds: Compound[], fromDate: Date = new Date(), logs: InjectionLog[] = []): { date: Date, compound: Compound } | null => {
    let earliest: { date: Date, compound: Compound } | null = null;
    const startFrom = new Date(fromDate);
    startFrom.setHours(0, 0, 0, 0);

    compounds.filter(c => !c.isArchived).forEach(c => {
        ...
        if (c.frequencyType === 'specific_days' && c.frequencySpecificDays) {
            ...
            const cLogs = logs.filter(l => l.compoundId === c.id).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
