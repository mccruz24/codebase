...
type ProtocolFormScreenProps = {
  mode: 'add' | 'edit';
};

export default function ProtocolFormScreen({ mode }: ProtocolFormScreenProps) {
  const scheme = useColorScheme();
  const isDark = scheme === 'dark';
  const { id } = useLocalSearchParams<{ id?: string }>();
  const isEdit = mode === 'edit';
  const editId = typeof id === 'string' && id.length > 0 ? id : undefined;
    ...
    <Screen>
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: tabBarHeight }]} showsVerticalScrollIndicator={false}>
        ...
        {isEdit ? (
          ...
          >
            <Text style={[styles.sectionLabel, { color: textMuted }]}>Protocol Identity</Text>
            <View style={styles.lockedIdentityRow}>
              <View style={[styles.lockedIdentityBadge, { backgroundColor: fieldBg }]}>
                <FontAwesome
                  name={CATEGORY_META[form.category].icon}
                  size={14}
                  color={textMuted}
                />
                <Text style={[styles.lockedIdentityBadgeText, { color: textPrimary }]}>
                  {CATEGORY_META[form.category].label}
                </Text>
              </View>
              <View style={[styles.lockedIdentityBadge, { backgroundColor: fieldBg, flex: 1 }]}>
                <Text
                  style={[styles.lockedIdentityBadgeText, { color: textPrimary }]}
                  numberOfLines={1}
                >
                  {form.name || 'Protocol'}
                </Text>
            ...
            {form.category === 'booster' && form.subCategory ? (
              <Text style={[styles.fieldHint, { color: textMuted }]}>
                Skin booster type: {form.subCategory}
              </Text>
            ) : null}
            <Text style={[styles.fieldHint, { color: textMuted }]}>
            ...
            {(Object.keys(CATEGORY_META) as CompoundCategory[]).map((cat) => {
              ...
              return (
                <AnimatedPressable
                  ...
                  style={[
                    styles.categoryBtn,
                    isCategoryLocked ? styles.categoryBtnLocked : null,
                    active
                      ? [
        ...
        >
          <Text style={[styles.sectionLabel, { color: textMuted }]}>
            {isEdit ? 'Inventory' : 'Identity'}
          </Text>

          {form.category === 'booster' && !isEdit ? (
            <View style={styles.fieldGroup}>
                  ...
                  return (
                    <AnimatedPressable
                      ...
                      style={[
                        ...
                        isEdit ? styles.boosterTypeChipLocked : null,
                        active
              ...
              {isEdit ? (
                <Text style={[styles.fieldHint, { color: textMuted }]}>
                  Skin booster type is locked after creation.
                </Text>
              ) : null}
            </View>
          ) : null}

          {form.category === 'microneedling' && !isEdit ? (
            ...
            <View style={styles.fieldGroup}>
              ...
              <TextInput
                ...
                onFocus={() => {
                  if (!isNameLocked) setShowNameSuggestions(true);
                }}
                style={[
                  styles.input,
                  { color: textPrimary, backgroundColor: fieldBg },
                  isNameLocked ? styles.inputLocked : null,
                ]}
              />
              {isNameLocked ? (
                <Text style={[styles.fieldHint, { color: textMuted }]}>
                  Protocol name is locked after creation.
                </Text>
              ) : null}
        ...
        {showReconstitution ? (
          ...
          {isEdit ? (
            <View style={[styles.fieldGroup, { marginTop: -2 }]}>
              <Text style={[styles.fieldLabel, { color: textMuted }]}>Protocol Name</Text>
              <View style={[styles.lockedValueRow, { backgroundColor: fieldBg }]}>
                <Text style={[styles.lockedValueText, { color: textPrimary }]} numberOfLines={1}>
                  {form.name}
                </Text>
              </View>
            </View>
          ...
          {isEdit && form.category === 'booster' && form.subCategory ? (
            <View style={styles.fieldGroup}>
              <Text style={[styles.fieldLabel, { color: textMuted }]}>Skin Booster Type</Text>
              <View style={[styles.lockedValueRow, { backgroundColor: fieldBg }]}>
                <Text style={[styles.lockedValueText, { color: textPrimary }]} numberOfLines={1}>
