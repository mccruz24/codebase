...
export const CheckIn: React.FC = () => {
        ...
        <div key={key} className="bg-stone-50 dark:bg-stone-800/50 rounded-[24px] p-5">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center space-x-2">
                <label className="text-xs font-bold text-stone-500 dark:text-stone-400 uppercase tracking-wider">{METRIC_LABELS[key as string]}</label>
                <button 
            ...
            <span className="text-sm font-bold text-white dark:text-stone-900 bg-stone-900 dark:bg-stone-100 w-8 h-8 rounded-full flex items-center justify-center shadow-lg shadow-stone-200 dark:shadow-stone-900/40">{ratings[key]}</span>
          </div>
  ...
  return (
    <div className="pt-2 pb-20 min-h-screen relative">
      {/* Info Modal */}
      {infoModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6 bg-stone-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setInfoModal(null)}>
              <div className="bg-white dark:bg-stone-900 p-6 rounded-[32px] max-w-xs w-full shadow-2xl animate-in zoom-in-95 duration-200 border border-stone-50 dark:border-stone-800" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="font-bold text-stone-900 dark:text-stone-100 uppercase tracking-wide text-sm">{METRIC_LABELS[infoModal]}</h3>
                      <button onClick={() => setInfoModal(null)} className="p-1 bg-stone-100 dark:bg-stone-800 rounded-full text-stone-500 dark:text-stone-400">
                          <X size={16} />
                      </button>
                  </div>
      ...
      <div className="flex items-center justify-between mb-6 px-1">
        ...
        <div className="text-center">
            <h1 className="text-xl font-bold text-stone-900 dark:text-stone-100">Log Your Progress</h1>
            <p className="text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wide">Health Journey Tracker</p>
        </div>
        <div className="w-8" />
      </div>
      ...
      <div className="space-y-6 animate-in slide-in-from-bottom duration-500">
         ...
         <div className="bg-white dark:bg-stone-900 p-8 rounded-[32px] shadow-sm border border-stone-50 dark:border-stone-800 text-center relative overflow-hidden">
             ...
             <label className="block text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-4">Current Body Weight</label>
         ...
         <div className="bg-white dark:bg-stone-900 p-6 rounded-[32px] shadow-sm border border-stone-50 dark:border-stone-800">
            <h3 className="text-lg font-bold text-stone-900 dark:text-stone-100 mb-6 px-1">Physical Aesthetics</h3>
            <SliderGroup keys={['muscleFullness', 'skinClarity', 'skinTexture', 'facialFullness', 'jawlineDefinition', 'inflammation']} />
         </div>

         {/* Well-being Section */}
         <div className="bg-white dark:bg-stone-900 p-6 rounded-[32px] shadow-sm border border-stone-50 dark:border-stone-800">
            <h3 className="text-lg font-bold text-stone-900 dark:text-stone-100 mb-6 px-1">Internal Well-being</h3>
