...
import { ChevronLeft, HelpCircle, Shield, Database, Activity, FileText, Syringe, TrendingUp } from 'lucide-react';
  ...
  return (
    <div className="pt-2 pb-20 min-h-screen bg-[#F8F9FB] dark:bg-stone-950 transition-colors duration-300">
      ...
      <div className="flex items-center justify-between mb-8 px-1">
        <button onClick={() => navigate(-1)} className="p-2 -ml-2 text-stone-500 dark:text-stone-400 rounded-full hover:bg-stone-100 dark:hover:bg-stone-900 transition-colors">
          <ChevronLeft size={24} />
        </button>
        <div className="text-center">
            <h1 className="text-xl font-bold text-stone-900 dark:text-stone-100">Help Center</h1>
            <p className="text-[10px] font-bold text-stone-400 uppercase tracking-wide">Support & FAQ</p>
        </div>
        <div className="w-8" />
      </div>
      ...
      <div className="space-y-8 animate-in slide-in-from-bottom duration-500">
        {sections.map((section, idx) => (
            <section key={idx}>
                <div className="flex items-center space-x-2 mb-4 px-2">
                    ...
                    <h2 className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest">{section.title}</h2>
                </div>
                ...
                <div className="bg-white dark:bg-stone-900 rounded-[32px] border border-stone-50 dark:border-stone-800 shadow-sm overflow-hidden">
                    {section.items.map((item, i) => (
                        <div key={i} className="p-6 border-b border-stone-50 dark:border-stone-800 last:border-0">
                            <h3 className="font-bold text-stone-900 dark:text-stone-200 text-sm mb-2">{item.q}</h3>
        ...
        <div className="px-4 py-4 text-center">
             ...
             <p className="text-xs font-bold text-stone-900 dark:text-stone-300 mt-1">Contact support at help@aestheticlog.com</p>
        </div>
      </div>
    </div>
  );
