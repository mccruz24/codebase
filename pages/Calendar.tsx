  ...
  const renderDays = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      days.push(
        ...
        >
          <span className={`text-sm font-bold w-8 h-8 flex items-center justify-center rounded-full transition-all ${isToday ? 'bg-stone-900 text-white shadow-lg' : 'text-stone-700 group-hover:scale-110'}`}>
            {d}
          </span>
          <div className="flex space-x-1 mt-2">
             {logs.length > 0 && <div className="w-1.5 h-1.5 rounded-full bg-stone-800" />}
             {scheduled.length > 0 && logs.length === 0 && <div className="w-1.5 h-1.5 rounded-full bg-pastel-blue" />}
             {checkIn && logs.length === 0 && scheduled.length === 0 && <div className="w-1.5 h-1.5 rounded-full bg-pastel-green" />}
          </div>
        </div>
      );
    }
    ...
    <div className="pt-6 pb-20 min-h-screen bg-white">
      ...
      <div className="px-4 mb-8">
        <div className="flex items-center justify-between bg-[#F8F9FB] p-2 rounded-[24px]">
           <button onClick={prevMonth} className="w-10 h-10 flex items-center justify-center bg-white rounded-xl shadow-sm text-stone-400 hover:text-stone-900">
             <ChevronLeft size={20} />
           </button>
           <h2 className="text-lg font-bold text-stone-900">
             {currentDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
           </h2>
           <button onClick={nextMonth} className="w-10 h-10 flex items-center justify-center bg-white rounded-xl shadow-sm text-stone-400 hover:text-stone-900">
