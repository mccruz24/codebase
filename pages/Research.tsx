    ...
    return (
      <div className="fixed inset-0 z-50 bg-[#F8F9FB] flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white p-8 rounded-[40px] shadow-xl border border-stone-50 max-w-md w-full animate-in fade-in zoom-in duration-300">
          ...
          <p className="text-sm text-stone-400 font-bold uppercase tracking-widest mb-6">Educational Reference Only</p>
          ...
          <div className="bg-stone-50 rounded-3xl p-6 text-left space-y-4 mb-8 border border-stone-100">
            <div className="flex items-start text-stone-600 text-sm leading-relaxed">
               ...
               <p>This section provides summaries of publicly available scientific literature for educational and research reference purposes only.</p>
            </div>
            <div className="flex items-start text-stone-600 text-sm leading-relaxed">
               <ShieldAlert size={18} className="mr-3 mt-1 shrink-0 text-stone-400" />
               <p>The information presented does not constitute medical advice, diagnosis, or treatment recommendations.</p>
            </div>
            <div className="flex items-start text-stone-600 text-sm leading-relaxed">
               <ShieldAlert size={18} className="mr-3 mt-1 shrink-0 text-stone-400" />
               <p>This app does not provide dosing guidance, protocol recommendations, or clinical interpretation.</p>
            </div>
          </div>
    ...
    <div className="pt-2 space-y-6 pb-20">
      ...
      <div className="space-y-3">
        {filteredEntries.map(entry => (
             <Link key={entry.id} to={`/research/${entry.id}`} className="block bg-white rounded-[28px] p-6 shadow-[0_2px_20px_rgb(0,0,0,0.04)] border border-stone-50 active:scale-[0.98] transition-all group">
                <div className="flex justify-between items-start mb-2">
                    <span className="bg-stone-50 text-stone-400 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider">{entry.category}</span>
                ...
                <p className="text-sm text-stone-400 font-medium line-clamp-2">{entry.classification}</p>
             </Link>
        ))}
      ...
      <div className="mt-8 px-4 text-center">
        <p className="text-[10px] text-stone-400 leading-relaxed max-w-xs mx-auto">
            This content is provided for educational reference only. The app does not provide medical advice, dosing guidance, or protocol recommendations.
        </p>
      </div>
    </div>
  );
