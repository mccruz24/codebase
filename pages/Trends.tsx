    ...
    <div className="pt-6 pb-24 min-h-screen">
      ...
      <div className="space-y-6 mb-8 px-1 animate-in slide-in-from-top duration-300">
        {METRIC_GROUPS.map((group) => (
          <div key={group.title}>
            <h3 className="text-[10px] font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-3 ml-1">
              {group.title}
            </h3>
      ...
      <div className="bg-white dark:bg-stone-900 rounded-[40px] shadow-sm border border-stone-50 dark:border-stone-800 h-[400px] relative overflow-hidden mb-8 flex flex-col transition-colors">
        <div className="p-8 pb-0 relative z-10">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-2">
                {getLabel(selectedMetric)}
              </p>
              <div className="flex items-baseline space-x-2">
                ...
                {selectedMetric !== 'weight' && (
                  <span className="text-lg font-bold text-stone-400 dark:text-stone-600">/ 10</span>
                )}
              </div>
            </div>
            ...
            {data.length > 1 && (
              <div
                className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-sm font-bold shadow-sm border border-stone-50 dark:border-stone-800 ${isPositive ? 'bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-200' : 'bg-white dark:bg-stone-800 text-stone-500 dark:text-stone-400'}`}
              >
                <span>
                  {isPositive ? '+' : ''}
                  {diff}
                ...
                <TrendingUp
                  size={14}
                  className={!isPositive ? 'text-stone-400 dark:text-stone-500' : ''}
                />
              </div>
            )}
          </div>
        ...
        <div className="flex-1 w-full mt-4 pr-6">
          {data.length < 2 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-8 opacity-40 mb-10">
              ...
              <p className="text-stone-400 dark:text-stone-600 text-sm font-bold">
