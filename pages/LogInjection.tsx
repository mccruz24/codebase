...
import { X, Check, Calendar, ChevronDown, ChevronLeft, ChevronRight, Syringe, Sparkles, Droplet, MapPin, Grid3X3, Camera, Image as ImageIcon } from 'lucide-react';
...
export const LogInjection: React.FC = () => {
  ...
  const handleCompoundSelect = (id: string, list = compounds) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const derivedSite = comp?.targetArea && comp.targetArea.length > 0 
        ? comp.targetArea.join(', ') 
        : undefined;
  ...
  const getIcon = (category?: string) => {
      switch(category) {
          case 'relaxant': return <Sparkles size={20} />;
          case 'booster': return <Droplet size={20} />;
          case 'microneedling': return <Grid3X3 size={20} />;
          default: return <Syringe size={20} />;
      }
  };
        ...
        <div className="h-full flex flex-col bg-[#F8F9FB] dark:bg-stone-950 min-h-screen -m-6 sm:m-0 fixed inset-0 z-50 overflow-y-auto animate-in fade-in duration-300">
            <div className="p-6 flex-1 flex flex-col max-w-md mx-auto w-full">
                <div className="flex justify-between items-center mb-6">
                    <button onClick={() => navigate(-1)} className="w-10 h-10 bg-white dark:bg-stone-900 rounded-full flex items-center justify-center text-stone-400 shadow-sm border border-stone-50 dark:border-stone-800">
                        <X size={20} />
                    </button>
                    <div className="w-10" />
                </div>
                ...
                <div className="flex-1 flex flex-col justify-center pb-20">
                    <h1 className="text-3xl font-bold text-stone-900 dark:text-stone-100 mb-2 text-center">Log Protocol</h1>
                    <p className="text-stone-400 text-center mb-10 font-medium">What did you administer today?</p>
                                    ...
                                    <div className="text-left">
                                        ...
                                        <div className="flex items-center space-x-2 mt-0.5">
                                            ...
                                            {c.targetArea && c.targetArea.length > 0 && (
                                                <span className="text-xs text-stone-400">• {c.targetArea[0]}{c.targetArea.length > 1 ? ` +${c.targetArea.length - 1}` : ''}</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                    ...
                    {compounds.length === 0 && (
                        <div className="text-center p-8">
                            ...
                            <button onClick={() => navigate('/compounds/new')} className="bg-stone-900 text-white px-8 py-4 rounded-2xl font-bold shadow-lg shadow-stone-200">
  ...
  return (
    <div className="h-full flex flex-col bg-[#F8F9FB] dark:bg-stone-950 min-h-screen -m-6 sm:m-0 fixed inset-0 z-50 overflow-y-auto animate-in slide-in-from-right duration-300">
        <div className="p-6 flex-1 flex flex-col max-w-md mx-auto w-full">
            <div className="flex justify-between items-center mb-6">
                <button onClick={handleBack} className="w-10 h-10 bg-white dark:bg-stone-900 rounded-full flex items-center justify-center text-stone-400 shadow-sm border border-stone-50 dark:border-stone-800">
                    <ChevronLeft size={20} />
                </button>
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
                ...
                <div className="mb-6 flex justify-center">
                    ...
                    >
                         <div className={`w-6 h-6 rounded-lg ${selectedCompound?.color} flex items-center justify-center text-white`}>
                             {selectedCompound?.category === 'relaxant' ? <Sparkles size={12} /> : selectedCompound?.category === 'booster' ? <Droplet size={12} /> : selectedCompound?.category === 'microneedling' ? <Grid3X3 size={12} /> : <Syringe size={12} />}
                         </div>
                         <span className="font-bold text-stone-900 dark:text-stone-100 text-lg">{selectedCompound?.name}</span>
                         <ChevronDown size={18} className="text-stone-300 group-hover:text-stone-500 transition-colors" />
                    </button>
                </div>
                ...
                <div className="bg-white dark:bg-stone-900 rounded-[40px] shadow-sm border border-stone-50 dark:border-stone-800 p-8 flex-1 flex flex-col space-y-8 relative overflow-hidden">
                     {/* Decorative Blob */}
                     <div className={`absolute top-0 left-0 w-full h-32 bg-gradient-to-b ${selectedCompound?.color.replace('bg-', 'from-').replace('500', '100')} to-transparent opacity-30`} />
                     ...
                     <div className="relative z-10 w-full text-center">
                        ...
                        {selectedCompound?.category === 'microneedling' ? (
                            ...
                            <div className="flex flex-col space-y-4">
                                <div>
                                    <label className="block text-center text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Needle Depth (mm)</label>
                                    <input
                                        type="number"
                                        inputMode="decimal"
                                        value={needleDepth}
                                ...
                                <div>
                                    <label className="block text-center text-xs font-bold text-stone-400 uppercase tracking-widest mb-2">Glide Serum</label>
                                    <input
                                        type="text"
                                        value={glideSerum}
                            ...
                            // STANDARD DOSE INPUT
                            <>
                                <label className="block text-center text-xs font-bold text-stone-400 uppercase tracking-widest mb-4">Dose Amount</label>
                                <div className="flex flex-col items-center justify-center">
                                    ...
                                    <span className="text-xl font-bold text-stone-400 mt-2">{selectedCompound?.doseUnit}</span>
                                </div>
                            </>
                        )}
                     </div>
                     ...
                     <div className="w-full space-y-6 relative z-10">
                        ...
                        <div className="bg-stone-50 dark:bg-stone-800 rounded-2xl p-4 border border-stone-100 dark:border-stone-700">
                            <label className="text-xs font-bold text-stone-400 uppercase tracking-wider mb-3 block">Before Photo</label>
                            <div className="flex items-center space-x-4">
                                <div 
                        ...
                        {selectedCompound?.targetArea && selectedCompound.targetArea.length > 0 && (
                             <div>
                                <div className="flex items-center space-x-2 mb-3 px-1">
                                    ...
                                    <label className="text-xs font-bold text-stone-400 uppercase tracking-wider">Treatment Site</label>
                                </div>
                                <div className="flex flex-wrap gap-2">
                                    {selectedCompound.targetArea.map(a => (
                                        <div key={a} className="bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 px-3 py-2 rounded-xl text-xs font-bold border border-stone-200 dark:border-stone-700">
                                            {a}
                                        </div>
                                    ))}
                                </div>
                ...
                <div className="pt-6 pb-6">
                    <button
                        type="submit"
                        disabled={!selectedCompoundId || (selectedCompound?.category !== 'microneedling' && !dose)}
