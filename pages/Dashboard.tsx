...
import { Activity, Check, ChevronRight, Scale, Syringe, Calendar as CalendarIcon, ArrowRight, Bell, Search } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  const [weekView, setWeekView] = useState<{ day: string, date: number, fullDate: Date, hasLog: boolean, hasSchedule: boolean, isToday: boolean }[]>([]);
  ...
  const [nextScheduled, setNextScheduled] = useState<{ date: Date, compound: Compound } | null>(null);
  ...
  useEffect(() => {
    ...
    setProgress(todayItems.length > 0 ? Math.round((todayItems.filter(i => i.isCompleted).length / todayItems.length) * 100) : 0);
  ...
  const weightChange = checkIns.length > 1 && checkIns[0].weight && checkIns[1].weight 
    ...
    <div className="space-y-6">
      ...
      <section>
         <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => navigate('/calendar')} className="p-2 bg-white dark:bg-stone-900 rounded-xl shadow-sm border border-stone-100 dark:border-stone-800">
         ...
         <div className="flex justify-between space-x-2 overflow-x-auto no-scrollbar pb-2">
             {weekView.map((d, i) => (
                 ...
                 >
                     ...
                     <div className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-bold ${d.isToday ? 'bg-stone-800 dark:bg-stone-200' : 'bg-stone-50 dark:bg-stone-800 text-stone-900 dark:text-stone-300'}`}>
                         {d.date}
                     </div>

                     <div className="mb-2">
                        {d.hasLog && <div className={`w-1.5 h-1.5 rounded-full ${d.isToday ? 'bg-white dark:bg-stone-900' : 'bg-stone-900 dark:bg-stone-300'}`} />}
                        {!d.hasLog && d.hasSchedule && <div className={`w-1.5 h-1.5 rounded-full ${d.isToday ? 'bg-stone-700 dark:bg-stone-300' : 'bg-stone-300 dark:bg-stone-700'}`} />}
                     </div>
                 </button>
             ))}
         </div>
      ...
      <section className="grid grid-cols-2 gap-4">
          ...
          >
              <div className="flex justify-between items-start mb-4 relative z-10">
                  <span className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wide">Weight</span>
                  <div className="w-8 h-8 rounded-full bg-pastel-green flex items-center justify-center text-stone-700">
                      <Scale size={14} />
                  </div>
              </div>
              <div className="relative z-10">
                  <h3 className="text-2xl font-bold text-stone-900 dark:text-stone-100">{currentWeight || '--'} <span className="text-sm font-medium text-stone-400 dark:text-stone-500">lbs</span></h3>
                  {weightChange && (
                      <div className="flex items-center mt-1 text-xs font-bold text-stone-500 dark:text-stone-400">
                          <span className="bg-stone-100 dark:bg-stone-800 px-1.5 py-0.5 rounded-md">{Number(weightChange) <= 0 ? '' : '+'}{weightChange}</span>
                          <span className="ml-1 text-[10px] text-stone-400 dark:text-stone-600 font-medium">Since last</span>
                      </div>
                  )}
              </div>
          ...
          <div className="bg-white dark:bg-stone-900 p-5 rounded-[28px] shadow-sm border border-stone-50 dark:border-stone-800 relative overflow-hidden active:scale-[0.98] transition-transform">
              <div className="flex justify-between items-start mb-4 relative z-10">
                  <span className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-wide">Up Next</span>
              ...
              <div className="relative z-10">
                  {scheduledToday.length > 0 && !scheduledToday.every(i => i.isCompleted) ? (
                      <>
                        <h3 className="text-xl font-bold text-stone-900 dark:text-stone-100 truncate">{scheduledToday.find(i => !i.isCompleted)?.compound.name}</h3>
                        <p className="text-xs font-bold text-stone-500 dark:text-stone-400 mt-1">Due Today</p>
                      </>
                  ) : nextScheduled ? (
                      <>
                        <h3 className="text-xl font-bold text-stone-900 dark:text-stone-100 truncate">{nextScheduled.compound.name}</h3>
                        <p className="text-xs font-bold text-stone-500 dark:text-stone-400 mt-1">
                             {nextScheduled.date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
                        </p>
                      </>
                  ) : (
      ...
      <section>
          <div className="flex justify-between items-center mb-4">
             ...
             <Link to="/log-injection" className="w-8 h-8 bg-stone-100 dark:bg-stone-800 rounded-full flex items-center justify-center text-stone-500 dark:text-stone-400 hover:bg-stone-200 dark:hover:bg-stone-700">
                 <ArrowRight size={16} />
             </Link>
          </div>
          ...
          <div className="space-y-3">
             {scheduledToday.length === 0 ? (
                 <div className="p-6 bg-white dark:bg-stone-900 rounded-3xl border border-stone-50 dark:border-stone-800 text-center">
                     <p className="text-stone-400 dark:text-stone-500 text-sm font-medium">Rest & Recovery</p>
                 </div>
             ) : (
                 scheduledToday.map(item => (
                    <div 
                       key={item.compound.id} 
                       onClick={() => !item.isCompleted && navigate(`/log-injection?compoundId=${item.compound.id}`)}
                    ...
                    >
                        <div className="flex items-center space-x-4">
                            <div className={`w-12 h-12 rounded-[18px] flex items-center justify-center ${item.isCompleted ? 'bg-stone-200 dark:bg-stone-800 text-stone-500' : `${item.compound.color.replace('bg-', 'bg-').replace('500', '100')} ${item.compound.color.replace('bg-', 'text-').replace('500', '600')}`}`}>
                                {item.isCompleted ? <Check size={20} /> : <Syringe size={20} />}
                            </div>
                            <div>
                                <p className={`font-bold ${item.isCompleted ? 'text-stone-400 line-through' : 'text-stone-900 dark:text-stone-100'}`}>{item.compound.name}</p>
                                <div className="flex items-center space-x-2 mt-0.5">
                                    <span className="text-xs font-bold text-stone-400">{item.dose} {item.compound.doseUnit}</span>
                                    {!item.isCompleted && <span className="w-1 h-1 rounded-full bg-stone-300 dark:bg-stone-600" />}
                                    {!item.isCompleted && <span className="text-xs text-stone-400">{item.compound.route}</span>}
                                </div>
                            </div>
                        </div>
                        {!item.isCompleted && (
