...
export const Compounds: React.FC = () => {
  ...
  const getCategoryIcon = (category?: string) => {
    switch (category) {
      ...
      case 'booster':
        return <Droplet size={20} strokeWidth={2.5} />;
      case 'microneedling':
        return <Grid3X3 size={20} strokeWidth={2.5} />;
      default:
        return <Syringe size={20} strokeWidth={2.5} />;
    }
  };
  ...
  const renderProtocolCard = (c: Compound) => (
    <Link
      key={c.id}
      to={`/compounds/edit/${c.id}`}
      className="block bg-white dark:bg-stone-900 rounded-[24px] p-5 shadow-[0_2px_20px_rgb(0,0,0,0.04)] border border-stone-50 dark:border-stone-800 active:scale-[0.98] transition-all relative overflow-hidden group mb-3"
    >
      ...
      {view === 'protocols' ? (
        ...
        <div className="space-y-6 animate-in slide-in-from-left duration-300">
          ...
          {groupedProtocols.booster.length > 0 && (
            <div className="relative">
              {(groupedProtocols.peptide.length > 0 || groupedProtocols.relaxant.length > 0) && (
                <div className="h-px bg-stone-100 dark:bg-stone-800 w-full mb-6" />
              )}
            ...
            <div className="space-y-4 pt-6">
              ...
              {archived.map((c) => (
                <Link
                  key={c.id}
                  to={`/compounds/edit/${c.id}`}
                  className="block bg-stone-100/50 dark:bg-stone-900/50 rounded-[24px] p-4 border border-transparent hover:border-stone-200 dark:hover:border-stone-800 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3 opacity-60">
                      ...
                      <p className="font-bold text-stone-600 dark:text-stone-400 line-through">
                        {c.name}
                      </p>
                    </div>
                ...
                <div className="space-y-3">
                  {groupedLogs[date].map((log) => {
                    ...
                    return (
                      <div
                        key={log.id}
                        className="bg-white dark:bg-stone-900 p-4 rounded-[24px] shadow-[0_2px_15px_rgb(0,0,0,0.03)] border border-stone-50 dark:border-stone-800 flex items-center justify-between"
                      >
                        <div className="flex items-center space-x-4">
                          <div
                            className={`w-10 h-10 rounded-2xl ${comp?.color || 'bg-stone-300'} flex items-center justify-center text-white shadow-sm shrink-0`}
                          >
                            {getCategoryIcon(comp?.category)}
                          </div>
                          <div>
                            <p className="font-bold text-stone-900 dark:text-stone-100 text-sm">
                              {comp?.name || 'Unknown Protocol'}
                            </p>
